...
const Doctor_POST = async (req, res) => {
  try {
    ...
    if (
      typeof rating_details.doctor != undefined &&
      rating_details.doctor != null
    ) {
      if (!checkForHexRegExp.test(rating_details.doctor)) {
        throw new Error("Faild to match required pattern for Doctor Id");
      } else {
        const doctor_count = await User.find({
          _id: rating_details.doctor,
          role_id: "doctor",
        }).count();
        if (doctor_count == 0) {
    ...
    if (
      typeof rating_details.rating != undefined &&
      rating_details.rating != null
    ) {
    ...
    if (
      typeof rating_details.type != undefined &&
      rating_details.type != null
    ) {
...
const List_POST = async (req, res) => {
  try {
    ...
    const rate = await RateSchema.find(cond)
      .populate("doctor_details")
      .populate("rater_details")
      .sort(sort)
      .skip((page - 1) * limit)
      .limit(limit);
